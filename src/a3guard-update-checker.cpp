...
void UpdateCheckerDialog::setupUI() {
...
void UpdateCheckerDialog::onFinished() {
    if (!m_reply) return;
...
void UpdateCheckerDialog::showError(const QString &msg) {
    m_progressBar->setRange(0, 1);
    m_progressBar->setValue(1);
    QMessageBox msgBox(this);
...
bool UpdateCheckerDialog::isNewerVersion(const QString &current, const QString &latest) const {
